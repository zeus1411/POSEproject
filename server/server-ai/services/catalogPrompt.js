const buildCatalogPrompt = ({ question, context, chatHistory = '' }) => {
  return [
    ...
    'Never mention Gemini, quota, rate limits, fallback handling, embeddings, retrieval, or internal system errors to the customer.',
    ...
    'Respect the requested quantity exactly. If the user asks for one product, answer with one product only. If the user asks for N products, list at most N products.',
    'If the user asks whether the store has a specific product, answer with the best matching product only unless the user explicitly asks for alternatives.',
