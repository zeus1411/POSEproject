...
// Import thêm StarIcon đặc từ thư mục solid để hiển thị sao vàng sắc nét giống ProductCard
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (user) dispatch(checkReviewStatus(id)); 
    }
  ...
  // ✅ ĐỒNG BỘ: Cải tiến bộ render sao sử dụng cả StarSolid để giao diện nhìn xịn hơn
  const renderStars = (rating) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < roundedRating) {
        stars.push(
          <StarSolid key={i} className="w-5 h-5 text-yellow-500 dark:text-yellow-400 fill-current" />
        );
      } else {
        stars.push(
  ...
  // ✅ ĐỒNG BỘ: Trích xuất chính xác biến dữ liệu từ DB giống ProductCard
  const averageRating = currentProduct.rating?.average || 0;
  const totalRatingCount = currentProduct.rating?.count || 0;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div>
                <h1 className="text-3xl font-bold text-white drop-shadow-[0_10px_24px_rgba(0,0,0,0.42)] mb-2">
                  {currentProduct.name}
                </h1>
                
                {/* ✅ SỬA LỖI TẠI ĐÂY: Truyền biến đã map đúng cấu trúc từ DB */}
                <div className="flex items-center gap-3 mb-4">
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="flex items-center gap-0.5">
                    {renderStars(averageRating)}
                  </motion.div>
                  <span className="text-sm font-medium text-cyan-200/90">
                    {averageRating > 0 ? `${averageRating.toFixed(1)} / 5.0` : 'Chưa có'} ({totalRatingCount} đánh giá)
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-6 border-t border-cyan-800/30">
                <div className="flex items-center gap-3">
                  <TruckIcon className="w-6 h-6 text-emerald-400" />
                  <div>
                    <p className="text-sm font-medium text-white/90">Miễn phí vận chuyển</p>
                    <p className="text-xs text-cyan-200/70">Cho đơn hàng từ 500k</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-white/90">Bảo hành chính hãng</p>
                    <p className="text-xs text-cyan-200/70">12 tháng</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-white/90">Đổi trả 30 ngày</p>
                    <p className="text-xs text-cyan-200/70">Miễn phí</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="glass-card mt-12 p-6 text-white bg-white/5 border border-cyan-300/10 rounded-2xl">
            <h2 className="text-2xl font-bold mb-4 text-white">Mô Tả Sản Phẩm</h2>
            <div 
              className="product-description-content text-cyan-100/90 leading-relaxed text-base"
              dangerouslySetInnerHTML={{ __html: currentProduct.description }}
            />
          </div>

          <div id="review-section" className="glass-card mt-12 p-6 text-white bg-white/5 border border-cyan-300/10 rounded-2xl">
