...
const fakeBlog = {
  _id: '1',
  title: 'Hướng dẫn setup hồ thủy sinh chuẩn đẹp như studio 🌿',
  content: `
    <p>Thủy sinh là một thú chơi rất chill và nghệ thuật...</p>
    <h2>1. Chọn bể</h2>
    <p>Bể kính nên chọn loại trong suốt, kích thước 60x30x36...</p>
    <h2>2. Ánh sáng</h2>
    <p>Dùng đèn LED chuyên dụng để cây phát triển tốt...</p>
    <img src="https://picsum.photos/800/400" />
    <h2>3. Setup layout</h2>
    <p>Có thể chọn style Iwagumi hoặc Nature...</p>
  `,
  coverImage: {
  ...
  relatedProducts: [
    {
      _id: 'p1',
      name: 'Đèn LED thủy sinh cao cấp',
      price: 450000,
      images: [{ url: 'https://picsum.photos/200?1' }]
    },
    {
      _id: 'p2',
      name: 'Phân nền ADA',
      price: 320000,
      images: [{ url: 'https://picsum.photos/200?2' }]
    }
  ]
};
...
const fakeComments = [
  {
    _id: 'c1',
    content: 'Bài viết xịn quá luôn 🔥',
    createdAt: new Date(),
    user: { name: 'Nguyễn Văn A' },
    replies: [
      {
        _id: 'r1',
        content: 'Chuẩn luôn bro!',
        createdAt: new Date(),
        user: { name: 'Admin' }
      }
    ]
...
const BlogDetailPage = () => {
    ...
    <div className={`flex gap-3 ${isReply ? 'ml-10 mt-3' : ''}`}>
      ...
      <div className="flex-1 group">
        ...
        <div className="flex gap-4 text-xs text-gray-500 mt-1 ml-2 opacity-0 group-hover:opacity-100 transition">
          <button
            onClick={() => handleLikeComment(c._id)}
            className={commentLikes[c._id] ? 'text-blue-500' : ''}
          >
            Like
          </button>
  ...
  if (!blog) return <div className="p-10 text-center">Loading demo...</div>;
  ...
  return (
    <div className="container mx-auto py-10 max-w-5xl">
      ...
      <img src={blog.coverImage.url} className="rounded-xl mb-6" />
        ...
        <div className="grid grid-cols-2 gap-4">
          {blog.relatedProducts.map((p) => (
            <div key={p._id} className="border rounded p-3">
              <img src={p.images[0].url} className="mb-2" />
              <p>{p.name}</p>
              <p className="text-red-500">{p.price.toLocaleString()}₫</p>
            </div>
          ))}
